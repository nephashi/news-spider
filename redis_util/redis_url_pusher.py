...
class RedisUrlPusher(object):
    ...
    def __init__(self, redis_queue_dao, logger):
    ...
    def log_cache_status(self):
        ...
        status += "num level: " + str(len(self.__crawled_urls)) + "; "
        for i in range(len(self.__crawled_urls)):
            status += "in " + str(i) + "th level, num url: " + str(len(self.__crawled_urls[i].items())) + "; "
